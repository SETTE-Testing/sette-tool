...
public final class ToolRegister {
    /** The set of tools. */
    private static final SortedSet<Tool> tools = new TreeSet<>();
     ...
     * @return the t
     */
    public static <T extends Tool> T get(Class<T> javaClass) {
    ...
    static void register(Tool tool) {
        ...
        if (another != null) {
            Validate.isTrue(false,
                    "Another tool with this type or name is already present "
                            + "(javaClass: [%s], name: [%s], "
                            + "another.javaClass: [%s], another.name: [%s])",
                    tool.getClass().getName(), tool.getName(), another.getClass().getName(),
                    another.getName());
        }
    ...
    public static Tool[] toArray() {
        return ToolRegister.tools.toArray(new Tool[ToolRegister.tools.size()]);
    }
}
