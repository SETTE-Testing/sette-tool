...
public final class ClassValidator extends AbstractValidator<Class<?>> {
    ...
    public void addException(String message, Throwable cause) {
        this.addException(new ClassValidationException(message, getSubject(), cause));
    }
     ...
     * @return this object
     */
    public ClassValidator type(ClassType type) {
        ...
        if (getSubject() != null) {
            ...
            switch (type) {
                case CLASS:
                    isTypeValid = !javaClass.isInterface() && !javaClass.isEnum()
                            && !javaClass.isAnnotation() && !javaClass.isPrimitive()
                            && !javaClass.isArray();
                    break;
                ...
                case REGULAR_CLASS:
                    isTypeValid = !javaClass.isInterface() && !javaClass.isEnum()
                            && !javaClass.isAnnotation() && !javaClass.isPrimitive()
                            && !javaClass.isArray() && !javaClass.isMemberClass()
                ...
                case ANONYMOUS_CLASS:
                    isTypeValid = !javaClass.isInterface() && !javaClass.isEnum()
                            && !javaClass.isAnnotation() && !javaClass.isPrimitive()
                            && !javaClass.isArray() && javaClass.isAnonymousClass();
                    break;

                case LOCAL_CLASS:
                    isTypeValid = !javaClass.isInterface() && !javaClass.isEnum()
                            && !javaClass.isAnnotation() && !javaClass.isPrimitive()
                            && !javaClass.isArray() && javaClass.isLocalClass();
                    break;
            ...
            if (!isTypeValid) {
                this.addException(String
                        .format("The Java class must have the specified type\n(type: [%s])", type));
            }
        }
        ...
        if (getSubject() != null) {
            ...
            if ((javaClass.getModifiers() & modifiers) != modifiers) {
                this.addException(String.format(
                        "The Java class must have all the "
                                + "specified modifiers\n(modifiers: [%s])",
     ...
     *
     * @param isSynthetic
     *            true if the Java class should be synthetic, false if it should not be
     * @return this object
     */
    public ClassValidator synthetic(boolean isSynthetic) {
        if (getSubject() != null) {
            Class<?> javaClass = getSubject();
     ...
     * @return this object
     */
    public ClassValidator superclass(Class<?> superclass) {
    ...
    public ClassValidator interfaceCount(int interfaceCount) {
        Validate.isTrue(interfaceCount >= 0,
                "The required interface count must be a non-negative number");
        ...
        if (getSubject() != null) {
            ...
            if (javaClass.getInterfaces().length != interfaceCount) {
                if (interfaceCount == 0) {
                    ...
                    this.addException(String.format(
                            "The Java class must implement exactly %d interfaces", interfaceCount));
                }
            }
        }
     ...
     * @return this object
     */
    public ClassValidator memberClassCount(int memberClassCount) {
        ...
        if (getSubject() != null) {
            ...
            if (javaClass.getClasses().length != memberClassCount) {
                if (memberClassCount == 0) {
                    ...
                    this.addException(
                            String.format("The Java class must have exactly %d member classes",
                                    memberClassCount));
                }
            }
        }
     ...
     * @return this object
     */
    public ClassValidator declaredConstructorCount(int declCtorCount) {
        ...
        if (getSubject() != null) {
            ...
            if (javaClass.getDeclaredConstructors().length != declCtorCount) {
                if (declCtorCount == 0) {
                    ...
                    this.addException(String.format(
                            "The Java class must have exactly %d declared constructors",
                            declCtorCount));
                }
            }
        }
