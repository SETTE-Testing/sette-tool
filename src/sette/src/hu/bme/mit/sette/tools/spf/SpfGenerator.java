    ...
    private void createGeneratedFiles() throws IOException {
        ...
        for (SnippetContainer container : getSnippetProject()
            ...
            for (Snippet snippet : container.getSnippets().values()) {
                ...
                jpfConfig.target = javaClass.getName() + '_'
                        + method.getName();

                String symbMethod = javaClass.getName()
                        + '.'
                        + method.getName()
                        + '('
                        + StringUtils.repeat("sym", "#",
                ...
                for (Class<?> parameterType : parameterTypes) {
                    parameterLiterals[i] = SpfGenerator
                            .getParameterLiteral(parameterType);
                    i++;
                }
                ...
                String relativePathJPF = relativePath + '.'
                        + JPFConfig.JPF_CONFIG_EXTENSION;
