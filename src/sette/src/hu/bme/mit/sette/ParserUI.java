...
public final class ParserUI {
    ...
    public ParserUI(SnippetProject snippetProject, Tool tool) {
        ...
        parser = tool.createRunResultParser(snippetProject,
                Run.OUTPUT_DIR);
    }
    ...
    public void run(BufferedReader in, PrintStream out)
            throws Exception {
        // directories
        File snippetProjectDir = parser.getSnippetProjectSettings()
                .getBaseDirectory();
        File runnerProjectDir = parser.getRunnerProjectSettings()
                .getBaseDirectory();
